// ============================================
// CONSULTATION.JS - VERSION AVEC TURN
// ============================================
...
(function() {
    ...
    const webrtc = {
        async createPeerConnection() {
            ...
            try {
                ...
                utils.log(`📡 Configuration ICE: ${config.iceServers.length} serveurs`);
                
                // Configuration avec TURN prioritaires
                state.peerConnection = new RTCPeerConnection({
                    iceServers: config.iceServers,
                    iceTransportPolicy: 'all',  // Utiliser TURN si nécessaire
                    iceCandidatePoolSize: 10,
                    bundlePolicy: 'max-bundle',
                    rtcpMuxPolicy: 'require'
                });
                
                // Timeout ICE (15 secondes max)
                ...
                state.peerConnection.onicecandidate = (event) => {
                    if (event.candidate && state.otherSocketId && state.socket) {
                        const candidateType = event.candidate.candidate.includes('relay') ? '🔄 TURN' : '🧊 STUN';
                        utils.log(`${candidateType} Candidat: ${event.candidate.type || 'host'}`);
                ...
                state.peerConnection.oniceconnectionstatechange = () => {
                    ...
                    if (iceState === 'connected' || iceState === 'completed') {
                        clearTimeout(iceTimeout);
                        utils.log('✅ ICE connecté via TURN/STUN');
        ...
        async handleFailure() {
            if (state.reconnectAttempts >= 3) {
                utils.log('❌ Abandon après 3 tentatives');
                utils.showToast('Connexion impossible. Vérifiez votre réseau.', 'error');
                return;
            }
            
            state.reconnectAttempts++;
            const delay = 5000 * state.reconnectAttempts;
            utils.log(`🔄 Nouvelle tentative dans ${delay/1000}s...`);
        ...
        async createOffer() {
            ...
            try {
                ...
                // Attendre ICE gathering (max 5 sec)
    ...
    // Initialisation
    async function init() {
        utils.log('🚀 Démarrage consultation v6.2...');
