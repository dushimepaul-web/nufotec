...
class Adresses extends MY_Controller {
    ...
    public function index()
    {
        ...
        // Construction de la requête avec jointures - CORRECTION des noms de colonnes pays
        $this->db->select('adresses.*, users.nom as user_nom, users.prenom as user_prenom, users.email as user_email, pays.pays as pays_nom, pays.ISO_3166_1_2_Letter_Code as pays_code');
        ...
        // Correction : utiliser le bon nom de colonne pour le pays
    ...
    public function Export()
    {
        ...
        // Correction : utiliser le bon nom de colonne pays
        $this->db->select('adresses.*, users.nom as user_nom, users.prenom as user_prenom, users.email as user_email, pays.pays as pays_nom, pays.ISO_3166_1_2_Letter_Code as pays_code');
        $this->db->from('adresses');
        $this->db->join('users', 'users.id = adresses.user_id', 'left');
        $this->db->join('pays', 'pays.id = adresses.pays_id', 'left');
        ...
        fputcsv($output, ['ID', 'Utilisateur', 'Email', 'Nom complet', 'Entreprise', 'Adresse', 'Complément', 'Code Postal', 'Ville', 'Pays', 'Code ISO', 'Téléphone', 'Type', 'Principale', 'Créé le']);
