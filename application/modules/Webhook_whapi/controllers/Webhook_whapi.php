...
class Webhook_whapi extends MX_Controller {
    ...
    public function index($token = null) {
        ...
        // Log du payload pour debug (à commenter en prod)
        log_message('info', 'Webhook reçu: ' . substr($raw_input, 0, 500));
        
        // Vérification du token (si dans l'URL)
        ...
        // Traiter le message (structure directe)
    ...
    private function process_message($payload) {
        ...
        // ==============================================
        // 2. EXTRAIRE LES DONNÉES (Structure Whapi réelle)
        // ==============================================
        ...
        // Vérifier aussi si admin dans le groupe
        ...
        if (!$is_admin) {
            ...
            // Membre simple avec texte OK - on laisse passer
        ...
        // Si ce n'est pas le groupe maître, on ne diffuse pas
        if (!$is_master_group) {
            log_message('debug', "Webhook: message admin dans groupe cible, pas de broadcast");
            return;
        }
        
        // Déterminer la cible (#groupe, #inbox, #template)
        $target_type = 'both';
