...
<!-- ═══════════════════════════════════════════════════════ -->
<!-- HERO CAROUSEL - ULTRA DYNAMIQUE & INTERACTIF -->
<!-- ═══════════════════════════════════════════════════════ -->
<style>
:root {
    --primary: #0f4c3a;
...
/* Hero Carousel - Style App Native avec gestures */
.hero-carousel {
    position: relative;
    width: 100%;
    overflow: hidden;
...
.hero-slide::after {
    ...
    right: 0;
    bottom: 0;
    background: linear-gradient(135deg, rgba(15, 76, 58, 0.92) 0%, rgba(15, 76, 58, 0.7) 50%, rgba(0,0,0,0.5) 100%);
    z-index: 1;
}
...
.hero-image-overlay {
    position: absolute;
    inset: 0;
    background: linear-gradient(to top, rgba(15, 76, 58, 0.9) 0%, transparent 50%);
    opacity: 1;
}

/* Carousel Navigation - Style Mobile App */
.carousel-dots {
    position: absolute;
    bottom: 100px;
    left: 50%;
...
@keyframes fadeInScale {
    from { opacity: 0; transform: perspective(1000px) rotateY(-10deg) scale(0.9); }
...
<section class="hero-carousel" id="heroCarousel">
    <div class="hero-carousel-container">
        ...
        <div class="hero-slide <?= $index === 0 ? 'active' : '' ?>" 
             data-index="<?= $index ?>"
             style="--slide-bg: url('<?= base_url('attachments/heros/' . $slide['background_image']) ?>')">
            
            <div class="hero-container">
                <div class="hero-content">
                    <?php if (!empty($slide['subtitle'])) : ?>
                    <span class="hero-subtitle"><?= htmlspecialchars($slide['subtitle']) ?></span>
                    <?php endif; ?>
                    
                    <h1 class="hero-title"><?= $slide['title']?></h1>
                    
                    <?php if (!empty($slide['description'])) : ?>
                    <p class="hero-description"><?= htmlspecialchars($slide['description']) ?></p>
                    <?php endif; ?>
                    
                    <div class="hero-actions">
                        <?php if (!empty($slide['button_primary_text'])) : ?>
                        <a href="<?= !empty($slide['button_primary_link']) ? base_url($slide['button_primary_link']) : '#' ?>" 
                           class="btn-hero-primary">
                            <?php if (!empty($slide['button_primary_icon'])) : ?>
                            <i class="<?= htmlspecialchars($slide['button_primary_icon']) ?>"></i>
                            <?php endif; ?>
                            <?= htmlspecialchars($slide['button_primary_text']) ?>
                        </a>
                        <?php endif; ?>
                        
                        <?php if (!empty($slide['button_secondary_text'])) : ?>
                        <a href="<?= !empty($slide['button_secondary_link']) ? base_url($slide['button_secondary_link']) : '#' ?>" 
                           class="btn-hero-secondary">
                            <?php if (!empty($slide['button_secondary_icon'])) : ?>
                            <i class="<?= htmlspecialchars($slide['button_secondary_icon']) ?>"></i>
                            <?php endif; ?>
                            <?= htmlspecialchars($slide['button_secondary_text']) ?>
                        </a>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <div class="slide-counter">
        <span class="current" id="currentSlide">01</span> / <?= str_pad(count($slides), 2, '0', STR_PAD_LEFT) ?>
    </div>
    ...
    <button class="carousel-arrow prev" id="prevBtn" aria-label="Slide précédent">
        <i class="bi bi-chevron-left"></i>
    </button>
    <button class="carousel-arrow next" id="nextBtn" aria-label="Slide suivant">
...
(function() {
    ...
    function init() {
        if (!elements.carousel || CONFIG.totalSlides === 0) return;
        ...
        elements.slides.forEach(slide => {
            ...
            if (bg && bg !== 'url()') {
                ...
                style.textContent = `
                    .hero-slide[data-index="${slide.dataset.index}"]::before {
                        background-image: ${bg};
                    }
                `;
    ...
    function bindEvents() {
        ...
        elements.dots.forEach((dot, index) => {
            dot.addEventListener('click', () => {
                if (index !== state.currentSlide && !state.isTransitioning) {
                    goToSlide(index);
                    resetAutoSlide();
                }
            });
        ...
        elements.carousel.addEventListener('mouseenter', pauseAutoSlide);
        elements.carousel.addEventListener('mouseleave', resumeAutoSlide);
    ...
    function startAutoSlide() {
        if (state.autoSlideInterval) return;
        state.isPaused = false;
        startProgressBar();
        state.autoSlideInterval = setInterval(goToNext, CONFIG.slideDuration);
    }
    
    function pauseAutoSlide() {
        if (window.matchMedia('(pointer: coarse)').matches) return; // Ne pas pause sur mobile
        state.isPaused = true;
        clearInterval(state.autoSlideInterval);
        state.autoSlideInterval = null;
        elements.progressBar.style.transition = 'none';
        elements.progressBar.style.width = elements.progressBar.offsetWidth + 'px';
    }
    ...
    function startProgressBar() {
        ...
        elements.progressBar.offsetHeight; // Force reflow
        elements.progressBar.style.transition = `width ${CONFIG.slideDuration}ms linear`;
        elements.progressBar.style.width = '100%';
    }
    
    function handleVisibilityChange() {
        document.hidden ? pauseAutoSlide() : resumeAutoSlide();
    }
    
    function goToNext() {
        const next = (state.currentSlide + 1) % CONFIG.totalSlides;
        goToSlide(next);
    }
    
    function goToPrev() {
        const prev = (state.currentSlide - 1 + CONFIG.totalSlides) % CONFIG.totalSlides;
        goToSlide(prev);
    }
    ...
    function goToSlide(index) {
        ...
        elements.dots[state.currentSlide].classList.remove('active');
    ...
    function updateCounter() {
        if (elements.currentSlideEl) {
            elements.currentSlideEl.textContent = String(state.currentSlide + 1).padStart(2, '0');
        }
    }
...
<?php else : ?>
<div class="hero-placeholder" style="min-height: 60vh; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%); color: white; margin-top: calc(-1 * (var(--header-height) + 56px)); padding-top: calc(var(--header-height) + 56px);">
