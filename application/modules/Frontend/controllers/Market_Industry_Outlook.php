...
class Market_Industry_Outlook extends Public_Controller {
    ...
    private function load_section_data($type, $options = [], $lang = null) {
        ...
        switch ($type) {
            ...
            case 'tendances_marche':
            case 'market_trends':
                $trends = $this->Model->read('tendances_marche', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($trends as &$trend) {
                    $trend['titre']       = $trend["titre_{$lang}"] ?? $trend['titre_fr'] ?? $trend['titre'] ?? '';
                    $trend['description'] = $trend["description_{$lang}"] ?? $trend['description_fr'] ?? $trend['description'] ?? '';
                    $trend['croissance']  = $trend['croissance'] ?? '';
                }
            ...
            case 'donnees_marche':
            case 'market_data':
                $market_data = $this->Model->read('donnees_marche', ['est_actif' => 1], 'categorie', 'ASC');
                foreach ($market_data as &$md) {
                    $md['categorie']     = $md["categorie_{$lang}"] ?? $md['categorie_fr'] ?? $md['categorie'] ?? '';
                    $md['description']   = $md["description_{$lang}"] ?? $md['description_fr'] ?? $md['description'] ?? '';
                }
                $data['market_data'] = $market_data;
                break;

            case 'analyse_concurrentielle':
            case 'competitive_analysis':
                $competitors = $this->Model->read('analyse_concurrentielle', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($competitors as &$comp) {
                    $comp['nom']         = $comp["nom_{$lang}"] ?? $comp['nom_fr'] ?? $comp['nom'] ?? '';
                    $comp['description'] = $comp["description_{$lang}"] ?? $comp['description_fr'] ?? $comp['description'] ?? '';
                    $comp['avantage']    = $comp["avantage_{$lang}"] ?? $comp['avantage_fr'] ?? $comp['avantage'] ?? '';
                }
                $data['competitors'] = $competitors;
                break;

            case 'opportunites_marche':
            case 'market_opportunities':
                $opportunities = $this->Model->read('opportunites_marche', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($opportunities as &$opp) {
                    $opp['titre']        = $opp["titre_{$lang}"] ?? $opp['titre_fr'] ?? $opp['titre'] ?? '';
                    $opp['description']  = $opp["description_{$lang}"] ?? $opp['description_fr'] ?? $opp['description'] ?? '';
                }
