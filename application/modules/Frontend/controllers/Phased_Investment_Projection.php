...
class Phased_Investment_Projection extends Public_Controller {
    ...
    public function index($lang = null) {
        ...
        // Note: Vue spécifique Phased_Investment_Projection
        $this->load->view('Phased_Investment_Projection', $data);
    }
        ...
        switch ($type) {
            ...
            case 'investment_cards':
                ...
                foreach ($phases as &$phase) {
                    ...
                    $phase['montant']     = $phase['montant'] ?? '';
                    $phase['periode']     = $phase['periode'] ?? '';
                }
                $data['phases'] = $phases;
                break;
            ...
            case 'allocation_fonds':
            case 'fund_allocation':
                $allocations = $this->Model->read('allocation_fonds', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($allocations as &$alloc) {
                    $alloc['categorie']    = $alloc["categorie_{$lang}"] ?? $alloc['categorie_fr'] ?? $alloc['categorie'] ?? '';
                    $alloc['description']  = $alloc["description_{$lang}"] ?? $alloc['description_fr'] ?? $alloc['description'] ?? '';
                    $alloc['pourcentage']  = $alloc['pourcentage'] ?? '';
                    $alloc['montant']      = $alloc['montant'] ?? '';
                }
                $data['fund_allocations'] = $allocations;
                break;

            case 'roi_projection':
            case 'return_on_investment':
                $roi_data = $this->Model->read('roi_projections', ['est_actif' => 1], 'annee', 'ASC');
                foreach ($roi_data as &$roi) {
                    $roi['description'] = $roi["description_{$lang}"] ?? $roi['description_fr'] ?? $roi['description'] ?? '';
                }
                $data['roi_projections'] = $roi_data;
                break;
