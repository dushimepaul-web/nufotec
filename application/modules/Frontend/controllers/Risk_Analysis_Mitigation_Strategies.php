...
class Risk_Analysis_Mitigation_Strategies extends Public_Controller {
    ...
    private function load_section_data($type, $options = [], $lang = null) {
        ...
        switch ($type) {
            ...
            case 'risques_mitigations':
            case 'risk_table':
                $risks = $this->Model->read('risques_mitigations', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($risks as &$risk) {
                    $risk['risque']        = $risk["risque_{$lang}"] ?? $risk['risque_fr'] ?? $risk['risque'] ?? '';
                    $risk['strategie']     = $risk["strategie_{$lang}"] ?? $risk['strategie_fr'] ?? $risk['strategie'] ?? '';
                    $risk['description']   = $risk["description_{$lang}"] ?? $risk['description_fr'] ?? $risk['description'] ?? '';
                }
                $data['risks'] = $risks;
                break;
