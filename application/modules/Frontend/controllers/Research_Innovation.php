...
class Research_Innovation extends Public_Controller {
    ...
    private function load_section_data($type, $options = [], $lang = null) {
        ...
        switch ($type) {
            ...
            case 'recherche_projets':
            case 'research_projects':
                $projects = $this->Model->read('recherche_projets', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($projects as &$project) {
                    $project['titre']       = $project["titre_{$lang}"] ?? $project['titre_fr'] ?? $project['titre'] ?? '';
                    $project['description'] = $project["description_{$lang}"] ?? $project['description_fr'] ?? $project['description'] ?? '';
                    $project['statut']      = $project["statut_{$lang}"] ?? $project['statut_fr'] ?? $project['statut'] ?? '';
                }
                $data['research_projects'] = $projects;
                break;

            case 'publications':
                $publications = $this->Model->read('publications', ['est_actif' => 1], 'date_publication', 'DESC');
                foreach ($publications as &$pub) {
                    $pub['titre']        = $pub["titre_{$lang}"] ?? $pub['titre_fr'] ?? $pub['titre'] ?? '';
                    $pub['resume']       = $pub["resume_{$lang}"] ?? $pub['resume_fr'] ?? $pub['resume'] ?? '';
                    $pub['auteurs']      = $pub["auteurs_{$lang}"] ?? $pub['auteurs_fr'] ?? $pub['auteurs'] ?? '';
                }
            ...
            case 'laboratoire':
            case 'laboratory':
                $lab_data = $this->Model->read('laboratoire_equipements', ['est_actif' => 1], 'ordre', 'ASC');
                foreach ($lab_data as &$item) {
                    $item['nom']         = $item["nom_{$lang}"] ?? $item['nom_fr'] ?? $item['nom'] ?? '';
                    $item['description'] = $item["description_{$lang}"] ?? $item['description_fr'] ?? $item['description'] ?? '';
                }
                $data['laboratory_equipment'] = $lab_data;
                break;
