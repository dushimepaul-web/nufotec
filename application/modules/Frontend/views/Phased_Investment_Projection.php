 ...
 * Vue dynamique pour les pages Frontend (type ESG, Vision, etc.)
 * 
 * Nécessite les variables : $page, $sections, $children, $lang (fournies par le contrôleur)
 */

// Sécurité : si $lang n'est pas définie (fallback)
if (!isset($lang) || empty($lang)) {
    $CI = &get_instance();
    $lang = $CI->current_lang ?? 'fr';
}
?>
...
/* ============================================
   CARTES PARTENAIRES (LISTE_CARD)
   ============================================ */
.partner-grid-section {
    background: linear-gradient(135deg, var(--light) 0%, var(--gray-soft) 100%);
}
.partner-card {
    background: white;
...
.partner-icon-wrapper {
    width: clamp(3rem, 10vw, 4.375rem);
    height: clamp(3rem, 10vw, 4.375rem);
...
.partner-tier-domestic {
    background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%);
    border-left-color: var(--success);
    color: #155724;
}
.partner-tier-regional {
    background: linear-gradient(135deg, #fff3cd 0%, #ffeeba 100%);
    border-left-color: var(--warning);
    color: #856404;
}
.partner-tier-international {
...
if (!function_exists('get_partner_icon')) {
...
if (!function_exists('parse_partner_cards')) {
    function parse_partner_cards($html_content) {
        ...
        foreach ($matches[1] as $li_html) {
            ...
            if (!empty($card['title'])) $cards[] = $card;
        }
        return $cards;
    }
}
    ...
    <?php foreach ($sections as $section): 
        ...
        switch($type):
            ...
            case 'liste_card':
                ...
                if (empty($cards)) {
                    echo '<section class="section ' . $custom_class . '"><div class="section-container"><div class="tinymce-content">' . $content . '</div></div></section>';
                    break;
                }
                ...
                $icon_colors = [
                    'microscope' => '#28a745', 'hospital' => '#dc3545', 'shield-check' => '#fd7e14',
                    'gear' => '#6c757d', 'shield-plus' => '#17a2b8', 'cash-coin' => '#ffc107', 'building' => '#0f4c3a'
                ];
                ?>
                                ...
                                <div class="<?php echo $col_class; ?> animate-item" style="--delay: <?php echo $index * 0.1; ?>s">
                                    <div class="partner-card h-100">
                                        ...
                                        <?php if (!empty($card['purpose'])): ?>
                                            <p class="partner-purpose"><i class="bi bi-bullseye me-2" style="color: var(--accent);"></i> <?php echo htmlspecialchars($card['purpose']); ?></p>
                                        <?php endif; ?>
                                        <div class="partner-tiers">
                                            <?php if (!empty($card['domestic'])): ?>
                                                <div class="partner-tier partner-tier-domestic">
                                                    <span class="partner-tier-label"><i class="bi bi-geo-alt-fill"></i> Domestic</span>
                                                    <div><?php echo htmlspecialchars($card['domestic']); ?></div>
                                                </div>
                                            <?php endif; ?>
                                            <?php if (!empty($card['regional'])): ?>
                                                <div class="partner-tier partner-tier-regional">
                                                    <span class="partner-tier-label"><i class="bi bi-globe-africa"></i> Regional</span>
                                            ...
                                            <?php if (!empty($card['international'])): ?>
                                                <div class="partner-tier partner-tier-international">
                                                    <span class="partner-tier-label"><i class="bi bi-globe"></i> International</span>
                        ...
                        <?php if (!empty($section['bouton_texte']) && !empty($section['bouton_lien'])): ?>
                            <div class="text-center mt-5 animate-item" style="--delay: <?php echo count($cards) * 0.1; ?>s">
                                <a href="<?php echo htmlspecialchars($section['bouton_lien']); ?>" class="btn btn-primary btn-lg rounded-pill">
                                    <?php echo htmlspecialchars($section['bouton_texte']); ?> <i class="bi bi-arrow-right ms-2"></i>
                                </a>
                            </div>
                        <?php endif; ?>
                    </div>
