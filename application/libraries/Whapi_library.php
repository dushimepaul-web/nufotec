...
class Whapi_library {
    ...
    private function load_settings() {
        ...
        foreach ($settings as $setting) {
            ...
            if ($key === 'api_token') {
                // CORRECTION : la valeur en base est une string simple (pas du JSON).
                // On tente le json_decode; si c'est du JSON avec clé 'token' on l'utilise,
                // sinon on prend la valeur brute directement.
                $raw        = $setting['setting_value'];
                $token_data = json_decode($raw, true);
                $this->api_token = (is_array($token_data) && isset($token_data['token']))
                    ? $token_data['token']
                    : $raw;
            } else {
    ...
    // CORRECTION : smart_delay() retiré de cette méthode.
    // Les délais doivent uniquement être dans le cron process_queue,
    // pas au moment de l'enqueue (appelé depuis le webhook = requête HTTP live).
    // ==============================================
    ...
    public function distribute_message($message_data, $sender_phone) {
        ...
        if ($target_type == 'group' || $target_type == 'both') {
            foreach ($groups as $group) {
                ...
                // Pas de smart_delay() ici — le délai est géré par le cron
            }
        }
    ...
    public function cleanup_deleted_groups() {
        ...
        foreach ($local_groups as $local_group) {
            if (!in_array($local_group->groupe_id, $remote_group_ids)) {
                $this->CI->db->where('groupe_id', $local_group->groupe_id)->delete('groupes_whatsapp');
                $this->CI->db->where('groupe_id', $local_group->groupe_id)->delete('whatsapp_participants');
                $deleted_count++;
            }
        }
