...
class Whapi_library {
    ...
    public function dequeue() {
        // Vérifier si la queue est en pause
    ...
    private function api_request($endpoint, $method = 'GET', $data = null) {
        ...
        throw new Exception("API request failed with code $http_code: $response");
    }
    ...
    public function distribute_message($message_data, $sender_phone) {
        ...
        // Règles de sécurité pour les membres simples (DANS TOUS LES GROUPES)
        if (!$is_admin) {
            // Bloquer les médias
            ...
            // Bloquer les numéros de téléphone
        ...
        // Si ce n'est pas le groupe maître, on ne diffuse pas (juste de la sécurité)
        if (!$is_master_group) {
            // C'est un message dans un groupe cible - on le laisse passer si c'est du texte simple
            return ['success' => true, 'type' => 'group_message', 'action' => 'allowed'];
        }
        
        // Ici, c'est le groupe maître - on diffuse
        // Récupérer les groupes actifs (CORRECTION: groupes_whatsapp au lieu de groupes_whattapp)
        ...
        if ($target_type == 'group' || $target_type == 'both') {
            foreach ($groups as $group) {
                ...
                // Délai intelligent entre groupes
                smart_delay();
            }
        }
